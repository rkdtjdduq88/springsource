...
/*
  하단의 페이지 번호 클릭 시
  a 태그 기본 기능 중지
  a 태그 href 값 가져온 후 operForm의 page요소의 value 값으로 세팅
  actionForm 전송
*/
...
pagination.addEventListener("click", (e) => {
  ...
  const page = document.querySelector("#operForm input:nth-child(1)");
  page.value = href;
...
window.onpageshow = function (event) {
  // persisted == true : webpage 가 로딩될 때 캐시(브라우저)에서 읽어왔음
  if (event.persisted) {
    location.reload();
  }
};
